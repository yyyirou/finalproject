...
<head>
    ...
    <link rel="stylesheet" href="assets/css/seller_1IVdel.css">
    <link rel="stylesheet" href="assets/css/seller_del.css">
    <link rel="stylesheet" href="assets/css/headerr.css">
    <link rel="icon" href="iimages/tai/icon.jpg" type="image/x-icon"  >
...
//Step 4: 執行 SQL 指令, 只有一筆資料          
			...
			sql = "SELECT * FROM `products_color` WHERE `pdid` = 'P009' AND `color`= '"+phcolor+"'";
			rs=con.createStatement().executeQuery(sql);
//Step 5: 顯示結果            
           while (rs.next()) //只有一筆資料
           {
			   ...
			   // 使用预处理语句构建SQL语句
				String updateSql = "UPDATE `products_color` SET `stock` = ? WHERE `pdid` = 'P009' AND `color` = ?";
    ...
    <main>
        <div class="imgg">
            <img class="IV" src="images/yi/Sony/1IV/IMG_3478.JPG" alt="1IV">
        </div>
		...
		<form method="post" action="" accept-charset="UTF-8">
            <div>
                ...
                <select class="choose" name="color">
                    ...
                    <option value="夜色黑" class="choose">夜色黑</option>
		...
		<form method="post" action="delete.jsp" accept-charset="UTF-8">
				<input type="hidden" name="productId" value="P009">
