...
<head>
    ...
    <link rel="stylesheet" href="assets/css/seller_6adel.css">
...
//Step 4: 執行 SQL 指令, 只有一筆資料          
			sql = "SELECT * FROM `products` WHERE `pdid` = 'P013'";
			...
			sql = "SELECT * FROM `products_color` WHERE `pdid` = 'P013' AND `color`= '"+phcolor+"'";
			rs=con.createStatement().executeQuery(sql);
//Step 5: 顯示結果            
           while (rs.next()) //只有一筆資料
           {
			   ...
			   // 使用预处理语句构建SQL语句
				String updateSql = "UPDATE `products_color` SET `stock` = ? WHERE `pdid` = 'P013' AND `color` = ?";
    ...
    <main>
		...
		<form method="post" action="delete.jsp" accept-charset="UTF-8">
				<input type="hidden" name="productId" value="P013">
