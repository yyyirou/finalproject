...
<head>
    ...
    <link rel="stylesheet" href="assets/css/seller_i13del.css">
...
//Step 4: 執行 SQL 指令, 只有一筆資料          
			...
			sql = "SELECT * FROM `products_color` WHERE `pdid` = 'P003' AND `color`= '"+phcolor+"'";
			rs=con.createStatement().executeQuery(sql);
//Step 5: 顯示結果            
           while (rs.next()) //只有一筆資料
           {
			   ...
			   // 使用预处理语句构建SQL语句
				String updateSql = "UPDATE `products_color` SET `stock` = ? WHERE `pdid` = 'P003' AND `color` = ?";
