...
<head>
    ...
    <link rel="stylesheet" href="assets/css/seller_reno8zdel.css">
...
//Step 4: 執行 SQL 指令, 只有一筆資料          
			sql = "SELECT * FROM `products` WHERE `pdid` = 'P008'";
			...
			sql = "SELECT * FROM `products_color` WHERE `pdid` = 'P008' AND `color`= '"+phcolor+"'";
			rs=con.createStatement().executeQuery(sql);
//Step 5: 顯示結果            
           while (rs.next()) //只有一筆資料
           {
			   ...
			   // 使用预处理语句构建SQL语句
				String updateSql = "UPDATE `products_color` SET `stock` = ? WHERE `pdid` = 'P008' AND `color` = ?";
		...
		<form method="post" action="" accept-charset="UTF-8">
            <div>
                ...
                <select class="choose" name="color">
                    ...
                    <option value="金" class="choose">金</option>
                    <option value="黑" class="choose">黑</option>
                </select>
            </div>
