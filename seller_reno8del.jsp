...
<head>
    ...
    <link rel="stylesheet" href="assets/css/seller_reno8del.css">
...
//Step 4: 執行 SQL 指令, 只有一筆資料          
			sql = "SELECT * FROM `products` WHERE `pdid` = 'P006'";
			...
			sql = "SELECT * FROM `products_color` WHERE `pdid` = 'P006' AND `color`= '"+phcolor+"'";
			rs=con.createStatement().executeQuery(sql);
//Step 5: 顯示結果            
           while (rs.next()) //只有一筆資料
           {
			   ...
			   // 使用预处理语句构建SQL语句
				String updateSql = "UPDATE `products_color` SET `stock` = ? WHERE `pdid` = 'P006' AND `color` = ?";
		...
		<form method="post" action="" accept-charset="UTF-8">
            <div>
                ...
                <select class="choose" name="color">
                    ...
                    <option value="緞面金" class="choose">緞面金</option>
                    <option value="微光黑" class="choose">微光黑</option>
                </select>
            </div>
